...
const snapshotPath =
  process.env.AFC_SNAPSHOT_PATH ||
  fileURLToPath(new URL(`../../../data/${validatorAddress}-snapshot.json`, import.meta.url));
const databasePath =
  process.env.AFC_DB_PATH ||
  fileURLToPath(new URL(`../../../data/${validatorAddress}-state.sqlite`, import.meta.url));
...
console.log(`AfroChain validator node listening on http://${binding.host}:${binding.port}`);
...
if (socketPort > 0) {
  ...
  console.log(`Validator socket transport listening on ${transport.publicUrl || transport.url}`);
}
...
if (autoBlockMs > 0) {
  setInterval(async () => {
    try {
      ...
      console.log(`Validator ${validatorAddress} awaiting proposer slot: ${error.message}`);
    }
  }, autoBlockMs);
}
...
if (syncMs > 0) {
  setInterval(async () => {
    try {
      ...
      console.log(
        `Validator sync ${summary.status}: +${summary.importedBlockCount} blocks, +${summary.importedTransactionCount} txs`
      );
    } catch (error) {
      console.log(`Validator sync failed: ${error.message}`);
    }
  }, syncMs);
}
